...
export default function ZapfPage() {
    ...
    try {
      const r = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({
          ...
          displayName: loginDisplayName || loginUsername
        })
      });
      if (r.ok) {
  ...
  // Polling alle 2 Sekunden für Queue und Stats
  useEffect(() => {
  ...
  async function fetchSalesPoints() {
    try {
      const r = await fetch("/api/sales-points");
      if (r.ok) {
        const data: SalesPoint[] = await r.json();
        setSalesPoints(data);
  ...
  async function fetchDrinks() {
    try {
      ...
      if (r.ok) {
        const data: Drink[] = await r.json();
        const pourNames = data.filter((x) => x.isPourDrink).map((x) => x.name);
        const uniqueButtons = new Set<string>(["Bier", "Radler"]);
        pourNames.forEach((n) => uniqueButtons.add(n));
        setButtonList(Array.from(uniqueButtons));
      }
    } catch {
      setButtonList(DEFAULT_POUR_BUTTONS);
    }
  }
  ...
  async function fetchQueue() {
    ...
    try {
      const r = await fetch(`/api/pour/queue?salesPointId=${selectedSalesPointId}`);
  ...
  async function fetchStats() {
    if (!selectedSalesPointId) return;
    try {
      const r = await fetch(`/api/pour/stats?salesPointId=${selectedSalesPointId}`);
      if (r.ok) {
        const data: StatItem[] = await r.json();
        const m = new Map<string, number>();
        data.forEach((i) => m.set(i.drinkName, i.totalPoured));
        setStats(m);
      }
    } catch (err) { console.error(err); }
  }

  // ✅ FIX: Robustere Zähler-Logik
  const getPendingCount = (btn: string) => {
    const bl = btn.toLowerCase().trim();
    let c = 0;
    rawQueue.forEach((q) => {
      if (q.pendingCount <= 0) return;
      const ql = q.drinkName.toLowerCase().trim();
      
      // Exakter Match oder sinnvolle Teilmengen (z.B. "Bier" matcht "Bier 0,4l")
      if (ql === bl || ql.includes(bl) || bl.includes(ql)) {
        c += q.pendingCount;
      }
    });
    return c;
  ...
  const getPouredCount = (btn: string) => {
    ...
    stats.forEach((p, n) => {
      ...
      // Exakter Match hat Priorität, sonst Teilmengen-Check
      if (nl === bl || nl.includes(bl) || bl.includes(nl)) {
        t += p;
      }
    });
    return t;
  ...
  const handleComplete = useCallback(async (dn: string) => {
    ...
    setRawQueue((prev) =>
      prev.map((q) => {
        const ql = q.drinkName.toLowerCase().trim();
        const dl = dn.toLowerCase().trim();
        const isMatch = ql === dl || ql.includes(dl) || dl.includes(ql);
        return isMatch && q.pendingCount > 0
          ? { ...q, pendingCount: q.pendingCount - 1 }
          : q;
      })
    );

    try {
      const r = await fetch("/api/pour/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ salesPointId: selectedSalesPointId, drinkName: dn })
      });
      
      if (r.ok) {
        setLastAction(`${dn} gezapft`);
        setTimeout(() => setLastAction(null), 1200);
        // Sofortiges Neuladen der Stats, damit der grüne Zähler hochgeht
        fetchStats();
        fetchQueue();
      } else {
    ...
    return (
      <div className="h-screen bg-gray-900 flex items-center justify-center p-4">
        <form onSubmit={handleLogin} className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
          ...
          <p className="text-xs text-gray-400 mb-6">Zapfen · Mitarbeiter-Anmeldung</p>
          {loginError && <div className="bg-red-900/50 text-red-300 text-sm p-3 rounded-lg mb-4 border border-red-700">{loginError}</div>}
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Account (Benutzername)</label>
              <input type="text" value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" autoFocus placeholder="z.B. team-festzelt" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Passwort</label>
              <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Dein Mitarbeiter-Name *</label>
              <input type="text" value={loginDisplayName} onChange={(e) => setLoginDisplayName(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" placeholder="z.B. Max, Anna, Tom" required />
            </div>
          ...
          <p className="text-[10px] text-gray-500 mt-4 text-center">Dein Name wird für die Zapf-Statistik verwendet.</p>
    ...
    <div className="h-screen flex flex-col bg-gray-900 text-white overflow-hidden select-none logo-watermark">
      <header className="flex items-center justify-between px-3 py-2 bg-gray-800 border-b border-gray-700 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {salesPoints.map((spItem) => (
              <option key={spItem.id} value={spItem.id}>{spItem.name}</option>
            ))}
          </select>
      ...
      <main className="flex-1 flex items-center justify-center p-3 gap-3 md:gap-6 overflow-y-auto">
        {(() => {
          const activeDrinks = buttonList.filter((d) => getPendingCount(d) > 0 || getPouredCount(d) > 0);
            ...
            <>
              <div className="flex flex-col gap-2.5 md:gap-4">
                ...
                {activeDrinks.map((drink) => {
                  const c = getPendingCount(drink);
                  return (
                    <div
                      key={`r-${drink}`}
                      className={`w-24 h-24 md:w-36 md:h-36 rounded-2xl flex flex-col items-center justify-center border-3 md:border-4 shadow-xl transition-all p-1.5 text-center ${
                        c > 0 ? "bg-red-600 border-red-400 animate-pulse scale-105" : "bg-red-900/40 border-red-800/60 opacity-80"
                      }`}
                    >
              ...
              <div className="flex flex-col gap-2.5 md:gap-4">
                ...
                {activeDrinks.map((drink) => {
                  const c = getPouredCount(drink);
                  const p = getPendingCount(drink);
                  return (
                    <button
                      key={`g-${drink}`}
                      onClick={() => handleComplete(drink)}
                      className={`w-24 h-24 md:w-36 md:h-36 rounded-2xl flex flex-col items-center justify-center bg-green-600 hover:bg-green-500 active:bg-green-700 active:scale-95 border-3 md:border-4 border-green-400 shadow-xl transition-all p-1.5 text-center ${
                        p === 0 ? "opacity-75 hover:opacity-100" : "ring-2 ring-green-300"
                      }`}
                    >
                      <span className="text-xs md:text-sm font-extrabold truncate w-full leading-tight">{drink}</span>
                      <span className="text-3xl md:text-5xl font-black mt-0.5">{c}</span>
      ...
      <footer className="shrink-0 bg-gray-800 border-t border-gray-700 px-3 py-1.5 text-center text-[11px] text-gray-400">
        TurboTap · Nur offene Getränke werden angezeigt · Grün tippen = 1 gezapft
      </footer>
    </div>
  );
}
