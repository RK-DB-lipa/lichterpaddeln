...
type Drink = { id: number; name: string; priceNet: number; taxRate: number; hasDeposit: boolean; depositAmount: number; cupSize: string; color: string; imageUrl: string | null; isActive: boolean; sortOrder: number; priceGross: number; isPourDrink: boolean; };
...
type SessionInfo = { authenticated: boolean; role?: string; username?: string; displayName?: string; tenantId?: number; };
...
export default function AdminPage() {
  ...
  const fetchDrinks = useCallback(async () => { try { const r = await fetch("/api/drinks"); if (r.ok) setDrinks(await r.json()); } catch (err) { console.error(err); } }, []);
  const fetchSalesPoints = useCallback(async () => { try { const r = await fetch("/api/sales-points"); if (r.ok) setSalesPoints(await r.json()); } catch (err) { console.error(err); } }, []);
  const fetchOrders = useCallback(async () => {
    try {
      ...
      if (qs) url += "?" + qs;
      const r = await fetch(url); if (r.ok) { const d = await r.json(); setOrders(d.orders || []); setDrinkSummary(d.drinkSummary || []); setTotals(d.totals || null); }
    } catch (err) { console.error(err); }
  }, [orderFilter, cashierFilter]);

  const fetchPourStats = useCallback(async () => { try { const r = await fetch("/api/pour/stats"); if (r.ok) setPourStats(await r.json()); } catch (err) { console.error(err); } }, []);
  const fetchCupCounters = useCallback(async () => { try { const r = await fetch("/api/cups"); if (r.ok) setCupCounters(await r.json()); } catch (err) { console.error(err); } }, []);
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (r.ok) { setSession({ authenticated: true }); setUsername(""); setPassword(""); }
      else { const d = await r.json(); setLoginError(d.error || "Anmeldung fehlgeschlagen"); }
  ...
  async function api(path: string, method: string, body?: any) {
    ...
    if (!r.ok) throw new Error((await r.json()).error || "API-Fehler"); return r.json();
  }
  ...
  async function handleSaveDrink(e: React.FormEvent) {
    ...
    try { const p = { name: formData.name, priceNet: parseFloat(formData.priceNet), taxRate: parseFloat(formData.taxRate), hasDeposit: formData.hasDeposit, depositAmount: parseFloat(formData.depositAmount), cupSize: formData.cupSize, color: formData.color, imageUrl: formData.imageUrl || null, sortOrder: parseInt(formData.sortOrder) || 0, isPourDrink: formData.isPourDrink };
      if (editingDrink) await api(`/api/drinks/${editingDrink.id}`, "PUT", p); else await api("/api/drinks", "POST", p); setShowDrinkForm(false); fetchDrinks(); }
  ...
  async function handleSaveSalesPoint(e: React.FormEvent) {
    e.preventDefault(); setSpFormError(""); if (!spFormName.trim()) { setSpFormError("Name erforderlich"); return; }
    try { await api("/api/sales-points", "POST", { name: spFormName.trim() }); setShowSalesPointForm(false); setSpFormName(""); fetchSalesPoints(); } catch (err: any) { setSpFormError(err.message); }
  }
  async function handleUpdateSalesPoint(e: React.FormEvent) { e.preventDefault(); if (!editingSalesPoint || !spEditName.trim()) return; try { await api(`/api/sales-points/${editingSalesPoint.id}`, "PUT", { name: spEditName.trim() }); setEditingSalesPoint(null); setSpEditName(""); fetchSalesPoints(); } catch (err) { console.error(err); } }
  async function handleDeleteSalesPoint(id: number) { if (!confirm("Verkaufsstelle deaktivieren?")) return; try { await api(`/api/sales-points/${id}`, "DELETE"); fetchSalesPoints(); } catch (err) { console.error(err); } }
  ...
  const showOrderDetail = async (order: Order) => {
    try {
      ...
      if (res.ok) { const d = await res.json(); setOrderDetail({ order: d.order, items: d.items || [] }); }
    } catch (err) { console.error(err); }
  };
  ...
  return (
    <div className="h-screen bg-gray-900 text-white flex flex-col overflow-hidden">
      ...
      <div className="flex border-b border-gray-700 shrink-0">
        {[["drinks","🍺 Getränke"],["salesPoints","🏪 Verkaufsstellen"],["cups","🥤 Becher"],["orders","📊 Bestellungen"],...(session.role === "admin" ? [["users","👥 Nutzer"]] : [])].map(([tab,label]) => (
          <button key={tab} onClick={() => setActiveTab(tab as any)} className={`flex-1 py-3 text-center font-bold text-sm ${activeTab === tab ? "border-b-2 border-blue-500 text-blue-400" : "text-gray-400"}`}>{label}</button>
        ))}
      </div>
      ...
      <div className="flex-1 p-4 max-w-4xl mx-auto w-full overflow-y-auto">
        {activeTab === "drinks" && (
          <div>
            ...
            ))}{drinks.length===0 && <p className="text-gray-500 text-center py-8">Keine Getränke angelegt.</p>}</div>
          </div>
        )}
        ...
        {activeTab === "salesPoints" && (
          <div>
            ...
            ))}{salesPoints.length===0 && <p className="text-gray-500 text-center py-8">Keine Verkaufsstellen.</p>}</div>
          </div>
        )}
        ...
        {activeTab === "cups" && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
              ...
              <div className="bg-gray-800 rounded-xl p-3 border border-gray-700"><div className="text-xs text-gray-400">0,2L im Umlauf</div><div className="text-2xl font-bold tabular-nums text-amber-400">{cupTotals.given02}</div></div>
              ...
              <div className="bg-gray-800 rounded-xl p-3 border border-gray-700"><div className="text-xs text-gray-400">0,4L im Umlauf</div><div className="text-2xl font-bold tabular-nums text-amber-400">{cupTotals.given04}</div></div>
            </div>
            ...
            {cupCounters.map((c)=>(
              <div key={c.salesPointId} className="bg-gray-800 rounded-xl p-3 border border-gray-700 mb-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-gray-700/50 rounded-lg p-2.5"><div className="text-[10px] text-gray-400 uppercase font-bold mb-1">0,2L</div>
                    ...
                    <div className="text-xs flex justify-between border-t border-gray-600 mt-1 pt-1"><span className="text-gray-400">im Umlauf:</span><span className={`font-bold tabular-nums ${c.given02 > 0 ? "text-red-400" : "text-green-400"}`}>{c.given02}</span></div>
                  </div>
                  <div className="bg-gray-700/50 rounded-lg p-2.5"><div className="text-[10px] text-gray-400 uppercase font-bold mb-1">0,4L</div>
                    ...
                    <div className="text-xs flex justify-between border-t border-gray-600 mt-1 pt-1"><span className="text-gray-400">im Umlauf:</span><span className={`font-bold tabular-nums ${c.given04 > 0 ? "text-red-400" : "text-green-400"}`}>{c.given04}</span></div>
                  </div>
                </div>
              </div>
            ))}
        ...
        {activeTab === "orders" && (
          <div>
            ...
            {/* Pour stats per drink type */}
            {pourStats.length > 0 && (
              <div className="mb-6 bg-gray-800 rounded-xl border border-gray-700 p-4">
                <div className="flex justify-between items-center mb-3"><h3 className="text-md font-bold">🍺 Zapf-Statistik (pro Getränkeart)</h3>
            ...
            <div className="mb-4 flex items-end gap-3 flex-wrap">
              <div className="flex-1 min-w-[150px]"><label className="block text-xs text-gray-400 mb-1">Verkaufsstelle</label>
              ...
              <div className="flex-1 min-w-[150px]"><label className="block text-xs text-gray-400 mb-1">Kassierer</label>
                <input type="text" value={cashierFilter} onChange={(e)=>setCashierFilter(e.target.value)} className="w-full bg-gray-700 text-white text-sm rounded-lg px-3 py-2 border border-gray-600 focus:border-blue-500 focus:outline-none" placeholder="z.B. Max" /></div>
            ...
            <h3 className="text-md font-bold mb-2">Einzelbestellungen {cashierFilter && <span className="text-gray-400 text-xs">(gefiltert: {cashierFilter})</span>}</h3>
            {orders.map((o)=>(
              <button key={o.id} onClick={()=>showOrderDetail(o)} className="w-full text-left bg-gray-800 rounded-xl p-3 border border-gray-700 mb-2 flex items-center justify-between hover:border-blue-500 transition-colors">
                <div><div className="font-bold text-sm">Bestellung #{o.id}</div>
                  <div className="text-xs text-gray-400">{new Date(o.createdAt).toLocaleString("de-DE")}</div>
                  <div className="text-xs text-blue-400 mt-0.5">{getSalesPointName(o.salesPointId)}</div>
                ...
                <div className="text-right"><div className="font-bold text-green-400">{o.totalGross.toFixed(2)} €</div>
                  <div className="text-xs text-gray-400">Pfand: {o.totalDeposit.toFixed(2)} €{o.totalDepositReturned>0 && ` (−${o.totalDepositReturned.toFixed(2)} € zurück)`}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-gray-800 rounded-2xl w-full max-w-md shadow-2xl border border-gray-600 max-h-[90vh] flex flex-col">
            <div className="p-4 border-b border-gray-700 shrink-0">
              <h2 className="text-xl font-bold text-center">🧾 Bestellung #{orderDetail.order.id}</h2>
              <p className="text-xs text-gray-400 text-center">{new Date(orderDetail.order.createdAt).toLocaleString("de-DE")}</p>
              <p className="text-xs text-blue-400 text-center">{getSalesPointName(orderDetail.order.salesPointId)}</p>
              {orderDetail.order.cashierName && <p className="text-xs text-green-400 text-center">👤 {orderDetail.order.cashierName}</p>}
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              {orderDetail.order.totalDeposit > 0 && (
                <div className="text-sm text-amber-400 flex justify-between pt-2"><span>Pfand (behalten)</span><span className="tabular-nums">+{orderDetail.order.totalDeposit.toFixed(2)} €</span></div>
              )}
              {orderDetail.order.totalDepositReturned > 0 && (
                <div className="text-sm text-red-400 flex justify-between"><span>Pfand zurück</span><span className="tabular-nums">-{orderDetail.order.totalDepositReturned.toFixed(2)} €</span></div>
              )}
              <div className="border-t border-gray-600 pt-2 mt-2 flex justify-between text-lg font-bold">
                <span>Gesamt</span><span className="text-green-400 tabular-nums">{orderDetail.order.totalGross.toFixed(2)} €</span>
              </div>
            </div>
            <div className="p-4 border-t border-gray-700 shrink-0">
              <button onClick={()=>setOrderDetail(null)} className="w-full py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-500 transition-all">Schließen</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Modals */}
      <DrinkFormModal show={showDrinkForm} editingDrink={editingDrink} formData={formData} formError={formError} setFormData={setFormData} handleSave={handleSaveDrink} onClose={()=>setShowDrinkForm(false)} />
      ...
      {editingSalesPoint && <SimpleModal title="Bearbeiten" error="" value={spEditName} onChange={setSpEditName} onSubmit={handleUpdateSalesPoint} onClose={()=>setEditingSalesPoint(null)} />}
      {showResetConfirm && <ConfirmModal title="⚠️ Zähler zurücksetzen?" onConfirm={handleResetOrders} onClose={()=>setShowResetConfirm(false)}><p className="text-sm text-gray-300 text-center">{resetTarget ? `Bestellungen für "${getSalesPointName(parseInt(resetTarget))}" löschen.` : "ALLE Bestellungen löschen."}</p></ConfirmModal>}
      {showPourResetConfirm && <ConfirmModal title="⚠️ Zapf nullen?" onConfirm={handleResetPour} onClose={()=>setShowPourResetConfirm(false)}><p className="text-sm text-gray-300 text-center">Zapf-Statistiken & Aufträge löschen.</p></ConfirmModal>}
      {showCupResetConfirm && <ConfirmModal title="⚠️ Becher nullen?" onConfirm={handleResetCups} onClose={()=>setShowCupResetConfirm(false)}><p className="text-sm text-gray-300 text-center">{cupResetTarget ? `Becher für "${getSalesPointName(parseInt(cupResetTarget))}" nullen.` : "ALLE Becher nullen."}</p></ConfirmModal>}
    </div>
  );
}

// --- Subcomponents ---
function DrinkFormModal({show,editingDrink,formData,formError,setFormData,handleSave,onClose}:any) {
  if (!show) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <form onSubmit={handleSave} className="bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-700 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm text-gray-400 mb-1">Netto (€)</label><input type="number" step="0.01" value={formData.priceNet} onChange={(e)=>setFormData({...formData,priceNet:e.target.value})} className="w-full px-4 py-2.5 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" /></div>
        ...
        <div className="flex gap-3 mt-6"><button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button type="submit" className="flex-1 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-500">Speichern</button></div>
      </form>
    </div>
  );
}
function SimpleModal({title,error,value,onChange,onSubmit,onClose}:any){return(
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
    <form onSubmit={onSubmit} className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
      ...
      <div className="flex gap-3"><button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button type="submit" className="flex-1 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-500">Speichern</button></div>
    </form>
  </div>
);}
function ConfirmModal({title,children,onConfirm,onClose}:any){return(
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
    <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
      ...
      {children}
      <div className="flex gap-3 mt-4"><button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button onClick={onConfirm} className="flex-1 py-3 rounded-xl font-bold bg-red-600 hover:bg-red-500">Ja</button></div>
    </div>
  </div>
);}
...
function UsersTab() {
  ...
  const [showCreate, setShowCreate] = useState(false); const [newName, setNewName] = useState(""); const [newPassword, setNewPassword] = useState(""); const [newDays, setNewDays] = useState("30"); const [createError, setCreateError] = useState("");
  ...
  async function handleCreate(e:React.FormEvent){e.preventDefault();setCreateError("");try{await api("/api/users","POST",{username:newName,password:newPassword,days:parseInt(newDays)});setShowCreate(false);setNewName("");setNewPassword("");setNewDays("30");reload();}catch(err:any){setCreateError(err.message||"Fehler");}}
  async function handleExtend(userId:number){try{await api("/api/users","PATCH",{userId,action:"extend",value:parseInt(extendDays)});setExtendingUser(null);reload();}catch(err){console.error(err);}}
  async function handlePassword(userId:number){try{await api("/api/users","PATCH",{userId,action:"resetPassword",value:pwValue});setPasswordUser(null);setPwValue("");}catch(err){console.error(err);}}
  async function handleDelete(userId:number,name:string){if(!confirm(`"${name}" wirklich löschen?`))return;try{await api("/api/users","DELETE",{userId});reload();}catch(err){console.error(err);}}
  async function toggleActive(userId:number){try{await api("/api/users","PATCH",{userId,action:"toggleActive"});reload();}catch(err){console.error(err);}}
  ...
  const dl = (e:string) => Math.ceil((new Date(e).getTime()-Date.now())/(24*60*60*1000));

  return (
    <div><div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold">Nutzer</h2><button onClick={()=>{setCreateError("");setShowCreate(true);}} className="px-4 py-2 rounded-xl bg-green-600 hover:bg-green-500 font-bold text-sm">+ Neuer</button></div>
      {users.map((u:any)=>{const left=dl(u.expiresAt);const exp=left<0;return(
        <div key={u.id} className="bg-gray-800 rounded-xl p-3 border border-gray-700 mb-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0"><div className="font-bold truncate flex items-center gap-2 flex-wrap">{u.username}{!u.isActive&&<span className="text-[10px] bg-gray-600 px-1.5 py-0.5 rounded text-white">DEAKT</span>}{exp&&<span className="text-[10px] bg-red-700 px-1.5 py-0.5 rounded text-white">ABGEL.</span>}</div>
              <div className="text-xs text-gray-400">bis {new Date(u.expiresAt).toLocaleDateString("de-DE")} · {exp?<span className="text-red-400 font-bold">abgelaufen</span>:<span className={`font-bold ${left<=7?"text-amber-400":"text-green-400"}`}>{left} T</span>}</div></div>
          </div>
          <div className="flex gap-1.5 mt-2.5 flex-wrap">
            <button onClick={()=>setExtendingUser(u)} className="px-3 py-1.5 rounded-lg bg-green-700 hover:bg-green-600 text-xs font-bold">⏳ Verl.</button>
            <button onClick={()=>setPasswordUser(u)} className="px-3 py-1.5 rounded-lg bg-gray-600 hover:bg-gray-500 text-xs font-bold">🔑 PW</button>
            <button onClick={()=>toggleActive(u.id)} className="px-3 py-1.5 rounded-lg bg-gray-600 hover:bg-gray-500 text-xs font-bold">{u.isActive?"⏸️":"▶️"}</button>
            <button onClick={()=>handleDelete(u.id,u.username)} className="px-3 py-1.5 rounded-lg bg-red-900/50 hover:bg-red-800/50 text-xs font-bold">🗑️</button>
          </div>
        </div>
      )})}
      {showCreate&&<CreateUserModal error={createError} name={newName} password={newPassword} days={newDays} onName={setNewName} onPassword={setNewPassword} onDays={setNewDays} onSubmit={handleCreate} onClose={()=>setShowCreate(false)} opts={opts}/>}
      {extendingUser&&<ExtendModal user={extendingUser} days={extendDays} onDays={setExtendDays} onSubmit={()=>handleExtend(extendingUser.id)} onClose={()=>setExtendingUser(null)} opts={opts}/>}
...
function CreateUserModal({error,name,password,days,onName,onPassword,onDays,onSubmit,onClose,opts}:any){return(
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><form onSubmit={onSubmit} className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
    <h2 className="text-xl font-bold mb-4">Neuer Nutzer</h2>
    {error&&<div className="bg-red-900/50 text-red-300 text-sm p-3 rounded-lg mb-4 border border-red-700">{error}</div>}
    ...
    <div className="mb-4"><label className="block text-sm text-gray-400 mb-1">Lizenz</label><select value={days} onChange={(e)=>onDays(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none">{opts.map((o:any)=>(<option key={o.d} value={o.d}>{o.l}</option>))}</select></div>
    <div className="flex gap-3"><button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button type="submit" className="flex-1 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-500">Anlegen</button></div>
  </form></div>
);}
function ExtendModal({user,days,onDays,onSubmit,onClose,opts}:any){return(
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><form onSubmit={onSubmit} className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
    <h2 className="text-xl font-bold mb-2">⏳ Verlängern</h2>
    <p className="text-sm text-gray-400 mb-4">{user?.username} · bis {new Date(user?.expiresAt).toLocaleDateString("de-DE")}</p>
    ...
    <div className="flex gap-3"><button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button type="submit" className="flex-1 py-3 rounded-xl font-bold bg-green-600 hover:bg-green-500">Verlängern</button></div>
  </form></div>
);}
function PasswordModal({user,value,onChange,onSubmit,onClose}:any){return(
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><form onSubmit={onSubmit} className="bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-gray-700">
    <h2 className="text-xl font-bold mb-2">🔑 Passwort</h2>
    <p className="text-sm text-gray-400 mb-4">für {user?.username}</p>
    <div className="mb-4"><input type="text" value={value} onChange={(e)=>onChange(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" autoFocus/></div>
    <div className="flex gap-3"><button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl font-bold bg-gray-600 hover:bg-gray-500">Abbrechen</button><button type="submit" className="flex-1 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-500">Speichern</button></div>
  </form></div>
);}
