...
export default function AdminPage() {
  ...
  }, [authenticated, fetchDrinks, fetchSalesPoints, fetchOrders]);
          ...
          <div>
            ...
            <div className="space-y-2">
              {drinks.map((drink) => (
                <div
                  key={drink.id}
                  className="bg-gray-800 rounded-xl p-3 flex items-center gap-3 border border-gray-700"
                >
                  <div
                    className="w-10 h-10 rounded-lg shrink-0 flex items-center justify-center font-bold text-white text-xs"
                  ...
                  <button
                    onClick={() => openEditDrinkForm(drink)}
            ...
            <div className="space-y-2">
              {salesPoints.map((sp) => (
                <div
                  ...
                  className="bg-gray-800 rounded-xl p-3 flex items-center gap-3 border border-gray-700"
                >
                  <div className="w-10 h-10 rounded-lg shrink-0 flex items-center justify-center font-bold text-white text-xs bg-blue-600">
                    {sp.name.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0">
              ...
              {salesPoints.length === 0 && (
                <p className="text-gray-500 text-center py-8">
                  Noch keine Verkaufsstellen angelegt.
                </p>
              )}
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ...
                <div className="bg-gray-800 rounded-xl p-3 border border-gray-700">
                  ...
                  <div className="text-2xl font-bold text-amber-400">
                    {(totals.totalDepositsCharged || 0).toFixed(2)} €
                  </div>
                </div>
                <div className="bg-gray-800 rounded-xl p-3 border border-gray-700">
                  <div className="text-xs text-gray-400">Pfand ausgezahlt</div>
                  <div className="text-2xl font-bold text-red-400">
                    -{(totals.totalDepositsReturned || 0).toFixed(2)} €
                  </div>
                </div>
              </div>
            )}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-blue-400 mt-0.5">
                        {getSalesPointName(order.salesPointId)}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-400">
                        ...
                        {order.totalDepositReturned > 0 &&
                          ` (−${order.totalDepositReturned.toFixed(2)} € zurück)`}
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {showDrinkForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <form
            ...
            className="bg-gray-800 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-gray-700 max-h-[90vh] overflow-y-auto"
          >
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundImage: formData.imageUrl
                      ? `linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.65)), url(${formData.imageUrl})`
                      : undefined,
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input
                type="text"
                value={spFormName}
                onChange={(e) => setSpFormName(e.target.value)}
                className="w-full px-4 py-2.5 rounded-xl bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none"
                placeholder="z.B. Getränkewagen 3"
                autoFocus
              />
            </div>
