...
export default function OrdersAnalytics() {
  ...
  const fetchSalesPoints = useCallback(async () => {
    try { const r = await fetch("/api/sales-points"); if (r.ok) setSalesPoints(await r.json()); } catch {}
  }, []);

  const fetchCashierNames = useCallback(async () => {
    try { const r = await fetch("/api/names?type=cashier"); if (r.ok) setCashierNames(await r.json()); } catch {}
  }, []);
    ...
    try {
      ...
      if (filterDate) {
        ...
        if (filterHour) {
          ...
          // Wenn Stunde 23, geht es bis Ende des Tages
  ...
  useEffect(() => { fetchSalesPoints(); fetchCashierNames(); }, [fetchSalesPoints, fetchCashierNames]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Detail-Modal für Einzelbestellung */}
      {selectedOrder && (
