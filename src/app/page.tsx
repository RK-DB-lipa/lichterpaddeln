...
export default function POSPage() {
      ...
      try {
        if ("wakeLock" in navigator) {
          ...
          wakeLockRef.current?.addEventListener("release", () => {
            // Re-request if released (e.g. tab switched)
            requestWakeLock();
          });
        }
      } catch {
  ...
  // Load sales point from localStorage
  useEffect(() => {
    const saved = localStorage.getItem("selectedSalesPointId");
    if (saved) {
      setSelectedSalesPointId(parseInt(saved));
    }
  }, []);

  // Save sales point to localStorage
  useEffect(() => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-900 text-white overflow-hidden">
      ...
      {/* Main area - drinks + deposit buttons */}
      <div className="flex flex-1 overflow-hidden" style={{ minHeight: 0 }}>
        {/* Deposit return buttons - left sidebar */}
            ...
            <div>
              ...
              <div className="space-y-0">
                {items.map((item) => (
                  <div
                    key={item.drinkId}
                    className="flex justify-between items-baseline text-xs md:text-sm border-t border-gray-700/40 py-0.5"
                  >
          ...
          <div className="bg-gray-800 rounded-2xl p-5 max-w-sm w-full shadow-2xl border border-gray-600">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl font-bold bg-gray-600 hover:bg-gray-500 text-sm
              ...
              <button
                onClick={handleReset}
                className="flex-1 py-2.5 rounded-xl font-bold bg-green-600 hover:bg-green-500 text-sm
                  active:scale-[0.97] transition-all border border-green-400/30"
              >
                Ja, abschließen
              </button>
            </div>
...
// Simple change calculator component
